...
int number[3] = {0, 0, 0}; // Array - each keystroke on the keypad is saved in coresponding array number
...
int turn = 0; // Used to decide if to play the next player tone or not (if a player wins we don/t play a tone while pressing next player -> POUND button)
...
void setup() {
	...
	for (j =0; j<6; j++){ // All 3 leds do a "breathing" animation while we wait for the keypad to initialize (6 seconds + code before that)
